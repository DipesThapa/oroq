...
(function(){
  ...
  function createHeuristicInsights(context){
    ...
    const pillText = `${riskLevel} • OroQ heuristics`;
    return {
      pillText,
      riskScore: safeScore,
  ...
  function evaluatePhishingSignals(options = {}){
    ...
    return {
      ...
      riskSummary: 'OroQ spotted suspicious login behavior.',
      signals
    };
  }
  ...
  function checkPhishingFeed(host, feedEntries){
    ...
    return {
      ...
      detail: `Known phishing host (${match.brand || 'unknown brand'}) per OroQ feed.`,
  ...
  function handlePhishingHit(phishingEval, pinConfig, audienceContext = {}){
    ...
    showInterstitial("Phishing protection", {
      ...
      supportLink: { text: 'Report phishing', href: 'https://dipesthapa.github.io/oroq/site/support.html' },
    ...
    window.fetch = async (...args)=>{
      try {
        ...
        if (method !== 'GET' && bodySample && shouldBlockPayload(bodySample) && !hostIsTrusted(url)){
          ...
          if (phishingEval){
            handlePhishingHit(phishingEval, pinConfig, context);
            return Promise.reject(new Error('Blocked by OroQ phishing guard'));
          }
        }
      } catch(_e){}
      ...
      xhr.send = async function(body){
        try {
          ...
          if (method !== 'GET' && bodySample && shouldBlockPayload(bodySample) && !hostIsTrusted(targetUrl)){
            ...
            if (phishingEval){
              handlePhishingHit(phishingEval, pinConfig, context);
              throw new Error('Blocked by OroQ phishing guard');
            }
          }
        } catch(err){
          if (err && err.message === 'Blocked by OroQ phishing guard') throw err;
        }
  ...
  function mask(el){
    ...
    try {
      ...
      tag.textContent = 'Blocked by OroQ';
  ...
  function showFullscreenOverlay(reason){
    try{
      ...
      const h = document.createElement('h2'); h.textContent = 'Blocked by OroQ'; h.style.margin='0 0 8px'; box.appendChild(h);
  ...
  function showInterstitial(reason, opts = {}){
    ...
    title.textContent = 'OroQ blocked this page';
    ...
    if (riskScore !== null){
      ...
      riskLevelEl.textContent = riskLevel || 'OroQ detection score';
      ...
      if (riskSummary || riskThreshold !== null){
        ...
        summaryText.textContent = riskSummary || 'OroQ combined signals exceeded the active threshold.';
    ...
    contextBits.push(`OroQ rule: ${reason || 'Policy enforcement'}`);
    ...
    if (ENABLE_INSIGHTS_PANEL){
      ...
      insightsTitle.textContent = 'What OroQ detected';
    ...
    if (pinRequired && !overrideDisabled){
      ...
      pinPrivacy.textContent = 'OroQ keeps this PIN on-device. Nothing is collected or shared.';
    ...
    const computedSupportUrl = (() => {
      ...
      return 'https://dipesthapa.github.io/oroq/site/support.html';
    })();
    ...
    support.appendChild(doc.createTextNode('Need to allow this permanently? Add it to your allowlist in the OroQ popup or '));
    ...
    if (focusActive && host){
      ...
      if (!focusAllowed){
        ...
        showInterstitial("Focus Mode", {
          ...
          riskSummary: 'OroQ blocked this site during Focus Mode to help you stay on-task.',
          riskPercent: 100,
          riskMax: 24,
          signals,
          disableOverride: overrideLocked
