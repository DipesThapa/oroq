...
- **Do you provide a way for users to request that their data is deleted?** → **Yes** — in-app: "Remove this device" unpairs and deletes that child's server data (`DELETE /pair/:id` clears the pairing row + its encrypted summary + command queue); plus an email deletion request to `dipesh@cyberheroez.co.uk`. Uninstall stops collection and removes local data; the server-side encrypted summary auto-expires in 7 days.
  - ✅ **Account deletion (resolved 2026-07-04):** in-app **More → Delete account** calls `DELETE /account` on the worker, which deletes every pairing (plus each pairing's KV summary + command queue), `push_tokens`, and the `accounts` row itself — verified deployed in production. The web deletion-request page required by Google's policy is live at **`https://oroq-site.pages.dev/delete-account`** — put that URL in the Data safety **Account deletion** field. (Self-serve web deletion via Google OAuth was considered and deferred post-launch; see `docs/superpowers/specs/2026-07-04-account-deletion-web-page-design.md`.)
- **Privacy policy URL:** `https://oroq-site.pages.dev/privacy-app` — live; `site/privacy-app.html` deployed to Cloudflare Pages (project `oroq-site`). `docs/PRIVACY_ANDROID.md` is the source-of-truth copy.
